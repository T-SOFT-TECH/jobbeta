<script lang="ts">
	...
	async function handleSubmit() {
		try {
			...
			if (isEditing) {
				await pb.collection('blog_posts').update(formData.id, data);
			} else {
			...
			alert('Failed to save blog post: ' + (e.message || 'Unknown error'));
		}
	}
...
<div>
	<div class="mb-6 flex items-center justify-between">
		<div>
			...
			<p class="mt-1 text-sm text-gray-600">Manage blog articles and content</p>
		</div>
		<button
			onclick={openCreate}
	...
	<DataTable {columns} data={posts} {loading} onEdit={openEdit} onDelete={handleDelete} />
		...
		>
			...
			<div>
				...
				<textarea
					...
					class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-purple-500 focus:outline-none focus:ring-purple-500"
				></textarea>
				<p class="mt-1 text-xs text-gray-500">Brief summary for preview (max 500 characters)</p>
			</div>
